...
// Fetches car listings from Supabase and renders them
...
function transformSupabaseCar(car) {
    ...
    // Transform to match frontend expected format (camelCase/PascalCase field names)
    return {
        id: car.id,
...
async function fetchCarsFromSupabase() {
    // Check if Supabase client is configured, retry initialization if needed
    if (!supabase) {
        // Try to initialize if not already done
        if (typeof window !== 'undefined' && window.supabase && typeof window.supabase.createClient === 'function') {
            supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
        } else {
            console.warn('Supabase client not configured. Please check config.js and ensure Supabase CDN is loaded. Static car cards will be displayed.');
    ...
    try {
        ...
        // Fetch cars with status 'Yayında' and join with car_images (explicit foreign key constraint: car_images_car_id_fkey)
        ...
        console.log('Total Records Fetched (Yayında only):', data ? data.length : 0);
        
        // Transform Supabase data to match frontend format
        let transformedRecords = (data || []).map(car => transformSupabaseCar(car));
        
        console.log('Records after transformation:', transformedRecords.length);
        
        // Store transformed records for further filtering
        allCars = transformedRecords;
...
function renderCarCards(records) {
    ...
    // Map Supabase fields to car card HTML
    records.forEach((record, index) => {
        ...
        // Get first image URL if Resim is an array (from transformSupabaseCar)
        let imageUrl = '';
